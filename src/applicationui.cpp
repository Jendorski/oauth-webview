...
void ApplicationUI::onSystemLanguageChanged()
{
    ...
    QString file_name = QString("OAuthWebView_%1").arg(locale_string);
    if (m_pTranslator->load(file_name, "app/native/qm")) {
        QCoreApplication::instance()->installTranslator(m_pTranslator);
    }
}

bool ApplicationUI::parseOAuthToken(QString url) {
	QUrl redirectUrl(url);
	if(!redirectUrl.hasFragment()) {
		return false;
	}
	QString frag = redirectUrl.fragment();
	if(frag.contains("&")) {
		QStringList args = frag.split("&");
		foreach(const QString query, args) {
			QStringList keyvalue = query.split("=");
			if(keyvalue.contains("access_token")) {
				// Do whatever with token
				return true;
			}
		}
	} else if(frag.contains("access_token")) {
		QStringList keyvalue = frag.split("=");
		if(keyvalue.contains("access_token")) {
			// Do whatever with token
			return true;
		}
